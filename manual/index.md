...
# カルステップの使用方法
カルステップ実演動画（short version）： [https://youtu.be/IMlzqnkYAkY](https://youtu.be/IMlzqnkYAkY)

カルステップ実演動画（詳細説明）： [https://youtu.be/40IkbkaXB9o](https://youtu.be/40IkbkaXB9o)
...
2.  **編集内容の転記**
    * **フットスイッチで転記**: 内容を編集した後、再度`Alt` + フットスイッチを押すと、**編集後の最新の内容**が転記されます。
...
* **過去の記録を一覧で確認**
    ...
    * 例えば、クラークやシュライバーがこの画面を確認し、迅速にカルテ作成を進める、といった連携が可能です。
...
4.  電子カルテなど、貼り付けたい場所にカーソルを合わせてペースト（`Ctrl+V`など）します。スプレッドシートの一番上の行にある最新の要約内容が貼り付けられます。
