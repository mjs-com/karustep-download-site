...
同じGoogleスプレッドシートを使用している別の診察室や別PCへ、患者情報と診察途中の内容を引き継げます。この機能は初期状態でONです。

1. 送り元の診察室で「一時停止」を押し、表示が変わったら「引き継ぐ」を押します。
2. 受け取り側でメニューの「別の診察室から引き継ぐ」を開きます。
3. 「一時停止の続き」から患者さんを選び、必要に応じて「詳細」で内容を確認します。
4. 「この患者を引き継ぐ」を押し、作成された新しいタブで録音を開始します。

診察済みの患者さんについても、「診察完了分」から過去の要約を事前情報として取り込めます。一時停止中の患者名と文字起こしは暗号化して一時保存され、1日を過ぎた情報は自動削除されます。使わない場合は「設定」→「アプリの設定」→「複数診察室での引き継ぎ」をOFFにしてください。
