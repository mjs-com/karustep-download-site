...
カルステップのアップデートは、新しいバージョンのアプリをZIPファイルとして配布しています。新規でご利用を始める方と同様、ダウンロードサイトからZIPをダウンロードして解凍するだけで利用できます。

**カスタムプロンプト（自分で作成・調整したプロンプト）を使っていない方**  
新しいZIPをダウンロードして解凍し、起動するだけでそのまま利用できます。（ライセンス・APIキーは登録済みのため、すぐに使い始められます。）

**カスタムプロンプトを使っている方**  
新しいZIPには前バージョンで作成したプロンプトは含まれていません。次の手順でプロンプトを移行してください。

1. 現在使用中のカルステップフォルダ内の **「programs」フォルダ** を開く。
   ![programsフォルダの場所](images/programsフォルダの場所提示.png)
2. 使用しているシステムプロンプト名の **テキストファイル** を探す。
3. そのファイルを **コピー** し、新しくダウンロードしたカルステップの **「programs」フォルダ** に貼り付ける。
4. アプリを **再起動** する。

再起動後、「診療科選択」に先生専用のプロンプトが表示されます。