...
3.  以下の情報を設定してください。
    ...
    - **リージョン**: リソースグループと同じリージョン（例：「Japan East」）を選択してください。
...
11. 同じ画面に表示されている「エンドポイント」のURLも右側にあるコピーアイコンをクリックしてコピーします。これら2つの情報は後ほどカルステップアプリに設定しますので、大切に保管してください。
...
もしゲーミングマウスの副ボタンやStream Deckでご使用になられる場合は「Ctrl + Shift + ,(カンマ)」を割り当ててください。

### 4. 記録内容の確認方法
要約内容は、いつでも簡単に確認することができます。
1.  カルステップのメイン画面にある「要約を見る」ボタンをクリックしてください。
2.  過去の記録データの一覧が表示され、任意の記録を閲覧できます。
音声ファイルや全文文字起こしの文章はtempフォルダ（"C:\temp"）に保存されます。ファイル名は録音時のタイムスタンプ（日付と時刻）となっています。
...
* **診療科や状況に応じた設定例**:
    ...
    * **内科（1対１）**: 医師と患者様の1対1の対話を主眼とし、AIが話者を誤認識しにくいように最適化されたプロンプトが設定されています。
