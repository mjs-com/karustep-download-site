...
2.  リストに表示されている「Azure サブスクリプション 1」（または類似の名称のサブスクリプション）をクリックします。
    ...
    **【重要】 サブスクリプションが一覧にない場合**
    - もし一覧にサブスクリプションが表示されない場合は、新規作成が必要です。その際、**プランの選択にご注意ください。**必ず**「従量課金」プラン**を選択（アップグレード）してください。
    「無料試用版」を選択すると、本サービスのAI機能（Azure OpenAI Service）がご利用いただけなくなります。
    - **ご安心ください。追加の費用負担はありません。**「従量課金」プランを選択した場合でも、登録から1ヶ月間有効な200ドル分の無料クレジットは適用されます。そして1か月後以降も実際に利用した分のみ料金が発生するプランのため、想定外の費用がかかることはありません。
