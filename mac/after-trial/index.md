...
5. 「端末名」へ、診察室Macなど識別しやすい名称を入力します。
6. 入力内容を確認し、「一括接続テスト」をクリックします。入力内容が保存された後、接続テストが実行されます。
7. `Speech: OK`、`OpenAI: OK`、`Sheets: OK`の3項目が表示されることを確認します。

APIキーはMacのキーチェーンへ安全に保存されます。添付されたExcelファイル、パスワード、APIキー、GoogleスプレッドシートIDは、第三者へ転送したり、スクリーンショットへ写したりしないでください。同じMacでアプリを更新しても再登録は通常不要です。別のMacへ移行する場合は、キーチェーンからAPIキーが自動移行されないため、移行先で再登録してください。
